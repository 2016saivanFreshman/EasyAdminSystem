...
//出现Allowed memory size of 134217728 bytes exhausted 这个错误都在此
function get_promptmsg($msgNum){			// 注：该方法存在一个死循环Bug。当prompt_msg表中不存在数据时，系统将无法获取到errormsg字段，之后引起递归函数递归调用，进入死循环
	$msgNum  = intval($msgNum);//变整
	
	$numargs = func_num_args();//这个函数返回的是当前函数的参数数量
    if ($numargs===0){
    ...
    $arg_list = func_get_args();//传回定义函数的参数列表的全部参数
    
    $data = MC("SELECT errormsg FROM ".C('DB_PREFIX')."prompt_msg WHERE id={$msgNum}", 1800);
	if (!isset($data[0]['errormsg'])){		// 死循环Bug出现在该处
	...
	return call_user_func_array('sprintf',$arg_list);//这是一个神奇的函数，用于执行参数不确定和函数回调，不太懂。第一个是回调函数，后面那个是参数
}
...
function json_return($message, $data=''){
	if (is_array($message)){
		...
		//array_keys() 函数返回包含数组中所有键名的一个新数组。就是把值都去掉了，留下键。下标为0 1 2 3
		$msgNo = array_shift(array_keys($message));
		// /implode() 函数返回由数组元素组合成的字符串。<br>放在元素之间
	    ...
	    if ($numargs<3){
	    	...
	    	//array_slice() 函数在数组中根据条件取出一段值，并返回。2代表从下标为2的开始取，会保留下标
		    $varlist  = array_slice($arg_list, 2);
		    //array_map(函数名，数组名)函数将用户自定义函数作用到数组中的每个值上，并返回用户自定义函数作用后的带有新值的数组。
	
		    //addslashes() 函数返回在预定义字符之前添加反斜杠的字符串预定义字符是：单引号（'）双引号（"）反斜杠（\）NULL
		    
		    //array2strarray把数组变成字符串数组
		    //eval()，把string变量当php代码执行
			$varstr   = implode(',', array2strarray(array_map('addslashes', array_map('htmlspecialchars', $varlist))));
		    eval("\$msg = get_promptmsg({$msgNo}, {$varstr});");
	    }
	}
	//json_encode，把数组转化为json格式
...
/**
 * 根据data（Array、STRING）生成SQL
 * @param array $data
 * @return string
 * @author jbf
 */
function buildWhereSql($data) {
	$where = '';
	if (is_array($data)) {
		foreach ($data AS $key => $value) {
			if (!empty($value) || $value === 0) $where .= "AND `".$key."` = '".$value."' ";
			
			unset($key); unset($value);
		}
		if (!empty($where)) $where = substr($where, 3);
	} else $where = $data;
