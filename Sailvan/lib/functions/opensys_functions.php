...
function callOpenSystem($paramArr){	
    ...
    if(!empty($paramArr['app_key'])){//POST方式
        $sign = createSign2($paramArr,$token);
    }else{
        $sign = createSign($paramArr,$token);
    }
	$strParam = array2http($paramArr)."&sign=".$sign; 	//生成签名,组织参数
	...
	$cnt=0;
    if(!empty($paramArr['app_key'])){//POST方式
        while($cnt < 3 && ($result=@vita_get_url_content2($urls, $paramArr))===FALSE) $cnt++;
    }else{//GET方式
    	echo "<br/>1651166";
