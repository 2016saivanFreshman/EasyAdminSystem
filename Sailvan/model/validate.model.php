...
class ValidateModel extends BaseModel{
    ...
    //mysql_real_escape_string() 函数转义 SQL 语句中使用的字符串中的特殊字符，防止注入
    protected function formatInsertField($table, $data){
    	...
    	unset($clist);
    	if (count($data)>0){
    		$this->validatemsg[10018] = get_promptmsg(10018, implode(',', array_keys($data)));
    	}
    ...
    private function _getTableColumns($table){
    	$formatcolumns = array();
    	$columnlists = $this->sql("SHOW COLUMNS FROM `{$table}`")->select(array('cache', 'mysql'), 3600);
